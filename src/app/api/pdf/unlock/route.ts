...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const response = await fetch(`${backendBase.replace(/\/$/, "")}/api/pdf/unlock`, {
      method: "POST", headers, body: req.body, duplex: "half",
    } as RequestInit & { duplex: "half" });
    const responseHeaders = new Headers();
    for (const name of ["content-type", "content-disposition", "cache-control", "x-password-required", "x-was-protected"]) {
